 ...
 * is not.
 *
 * The span keeps the original nodes, so `inline code` still renders as code on
 * the page. The spoken form ("inline code") is recomputed from those same
 * nodes at generation time by `speechOf`.
 *
 * Ported from zed-rust-for-dummies's src/plugins/rehype-tts.mjs — same
 * mechanism, no site-specific logic here.
 */
...
const BLOCKS = new Set([
  'p', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'dd', 'dt', 'td', 'th', 'figcaption', 'summary', 'blockquote',
]);

/** Subtrees that are never spoken. This site's quiz UI (RecallQuiz.astro)
 * renders under `.recall-quiz` — its question/feedback text isn't book prose
 * and was never meant to be read aloud. */
const SKIP = new Set(['script', 'style', 'svg', 'button']);
const SKIP_CLASS = new Set(['recall-quiz']);

const hasClass = (node, cls) => {
  const classes = node.properties?.className ?? [];
  return (Array.isArray(classes) ? classes : [classes]).map(String).includes(cls);
};
...
export default function rehypeTts() {
  return (tree) => {
    ...
    visit(tree, 'element', (node, _index, parent) => {
      if (node.tagName && [...SKIP_CLASS].some((cls) => hasClass(node, cls))) return 'skip';
