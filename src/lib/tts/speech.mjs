 ...
 *
 * A rules engine for the Rust/path/identifier grammar, plus a small
 * dictionary for punctuation and primitives the rules can't reach cleanly.
 *
 * Guiding rule: verbose where it enhances clarity, terse where verbose sounds
 * goofy. So `&mut Window` becomes "mutable reference to Window" — the long
 * form is what a person would say out loud.
 *
 * Ported from zed-rust-for-dummies's src/lib/tts/speech.mjs — the DICTIONARY
 * there was tuned for that book's own vocabulary (gpui, cx, zed identifiers);
 * this one keeps only the generic Rust/punctuation entries and drops the
...
function expandPath(s) {
  ...
  const spoken = path
    ...
    .filter(Boolean)
    .map((seg) => {
      // `{lib,main}.rs` — a brace group names sibling files.
...
export function speakCode(raw) {
  ...
  // Brace expansion: `Foo::{bar, baz}` → `Foo's bar, baz`.
  ...
  // Paths: `Foo::bar` → `Foo's bar`. A leading `::` (as in `::default()`)
  // has no owner to be possessive about — drop it.
  s = s.replace(/(\w)::(\w)/g, "$1's $2").replace(/^::/, '');

  // Closure parameter lists: `|x, y|` is not a disjunction.
  ...
  // Operators. `=>` is matched before `=` so an arrow doesn't come out as
  // "equals greater than".
 ...
 *
 * Reading a code block line by line would be unlistenable, but silently
 * dropping it loses the fact that the prose is pointing at something.
 *
 * Expressive Code (if in use) rewrites `pre` blocks so the language moves to
 * `pre[data-language]` and each source line becomes its own `div.ec-line`.
 * This site doesn't use Expressive Code, so the fallback (plain `<pre><code
 * class="language-x">`, counting newlines) is what actually fires — the
 * `ec-line` path is kept for parity in case that changes later.
 *
...
export function announceCodeBlock(pre) {
  ...
  const named = {
    ...
    toml: 'TOML', diff: 'diff', text: '', plaintext: '', console: 'console',
  };
